...
class Run:
    ...
    def __init__(self, input_folder: Path,
                 ...
                 output_folder: Path = False,
                 experiment_folder: Path = False):
        ...
        # if output_folder was provided, get run name from folder
        if output_folder:
            self.run_name: str = output_folder.name

        # if experiment_folder was provided, get run name from parameters
        elif experiment_folder:
            # get the function name according to the 'name_function' parameters
            run_name_function_params: dict = general_parameters['name_function']

            # initialize the name function given the set of parameters
            def run_name_function(params: dict, variables: dict):
                name: str = ''
            ...
            # set the run name
            self.run_name: str = run_name_function(
                run_name_function_params, self.variables)

            # create output folder path object
            output_folder: Path = experiment_folder / self.run_name
        else:
        ...
        self.successful: bool = self.get_success_status()
    ...
    def initialize_paths(self, input_folder, output_folder, general_parameters):
        ...
        run_paths['input'] = f"{input_folder}"
        return run_paths
            ...
            if temp_folder_list:
                for folder in temp_folder_list:
                    ...
                    if correct_folder:
                        ...
                        logging.info(
                            f'Test file {test_files[key]} found in folder {folder}')
                        continue
        return temp_paths
...
class RunSubmitter:
    ...
    def update_paths(self, paths: dict):
        ...
        paths['slurm_output'] = f"{output_folder}/{self.run.run_name}"
        return paths
