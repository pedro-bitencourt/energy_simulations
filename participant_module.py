...
class Participant:
    ...
    def get_production_data(self, daily: bool = False) -> Optional[pd.DataFrame]:
        # construct the path to save the results
        ...
        if not os.path.exists(production_output_path):
            ...
            # save the extracted production data
            dataframe.to_csv(production_output_path, index=False)
            logging.info(
                f"Successfully extracted and saved {self.key} production data.")
        else:
            # if output already exists, read the production data from the saved file
            dataframe = pd.read_csv(production_output_path)
            logging.info(
                f"Successfully read {self.key} production data from {production_output_path}")
        return dataframe
    ...
    def process_participant(self, daily: bool = False) -> Optional[Dict[str, float]]:
        ...
        if participant_df is None:
            logging.error(
                f'Error while extracting production data for {self.key}')
            return None
