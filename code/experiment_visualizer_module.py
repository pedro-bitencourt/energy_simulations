...
EXOG_VAR: str = 'lake_factor'
EXOG_VAR_LABEL: str = 'Lake Factor'
...
class ExperimentVisualizer:
    ...
    def plot_pca_price_distributions(self):
        ...
        components_df[EXOG_VAR] = [self.dict_of_run_results[run]
                                         .results_dict[EXOG_VAR]
                                         for run in runs]
    ...
    def plot_intraweek_price_distributions(self):
        ...
        for run_results in self.dict_of_run_results.values():
            ...
            if 'thermal_capacity' in results_dict:
                title = f"{EXOG_VAR_LABEL}: {results_dict[EXOG_VAR]:.1f}, Thermal capacity: {results_dict['thermal_capacity']:.1f}"
            else:
                title = f"{EXOG_VAR_LABEL}: {results_dict[EXOG_VAR]:.1f}"
...
HEATMAP_CONFIGS = [
    {'variables': {'x': {'key': EXOG_VAR, 'label': EXOG_VAR_LABEL},
...
ONE_D_PLOTS_CONFIGTS = [
    # Optimal capacities of wind and solar
    {
        ...
        'axis_labels': {'x': EXOG_VAR_LABEL, 'y': 'Capacity (MW)'},
    ...
    # Total production by resources
    {
        ...
        'axis_labels': {'x': EXOG_VAR_LABEL, 'y': 'Total Production (GWh)'},
...
#    },
    # Average price
    {
        ...
        'axis_labels': {'x': EXOG_VAR_LABEL, 'y': 'Price ($/MWh)'},
        'title': 'Average Price',
