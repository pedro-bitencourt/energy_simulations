...
def read_xlt(file_path, options):
    ...
    if table_text == READING_ERROR:
        print(f"Reading error while reading {file_path}.")
        return False
...
#def sum_discounted_products(dataframe, variables, discount_factor_col='discount_factor'):
#    def process_variable(variable):
#        # Convert the variable column to numeric, coercing errors to NaN
#        numeric_values = pd.to_numeric(dataframe[variable], errors='coerce')
#
#        # Calculate the product of the variable and the discount factor
#        return numeric_values * dataframe[discount_factor_col]
#
#    if isinstance(variables, list):
#        # If variables is a list, process each variable
#        products = [process_variable(var) for var in variables]
#
#        # Sum all products
#        total_sum = sum(product.sum() for product in products)
#    else:
#        # If variables is a single string, process just that variable
#        products = process_variable(variables)
...
def get_present_value(dataframe: pd.DataFrame,
    ...
    if log:
        logging.debug("Getting present value of variable %s for %d years", variable, years)

    # Convert datetime and calculate days difference
    days_diff = (pd.to_datetime(dataframe['datetime']) - reference_date).dt.days

    # Calculate discount factor and present value
    discount_factor = 1 / (1 + daily_rate)**days_diff
    present_values = pd.to_numeric(dataframe[variable], errors='coerce') * discount_factor
