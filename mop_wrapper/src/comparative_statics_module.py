...
class ComparativeStatics:
    ...
    def _initialize_solvers(self):
        ...
        """
        grid_points = []
        exogenous_variable, grid = list(self.variables['exogenous'].items())[0]
        for value in grid:
            grid_points.append(self._construct_solver(exogenous_variable, value))
        return grid_points
...
''')
    ...
    def process(self, complete=True, resubmit=True):
        ...
        solver_results(list_of_solvers=self.solvers).to_csv(self.paths['solver_results'], index=False)
